...
public class War extends JavaPlugin {
	...
	private Map<OfflinePlayer, Set<OfflinePlayer>> warMongers = Collections
			.synchronizedMap(new HashMap<OfflinePlayer, Set<OfflinePlayer>>());
	 ...
	 * 
	 * @param monger
	 *            The person initiating the war
	 * @param target
	 *            The person upon which the war is being declared
	...
	public boolean addWar(OfflinePlayer monger, OfflinePlayer target) {
		if (!warMongers.containsKey(monger)) {
			warMongers.put(monger,
					Collections.synchronizedSet(new HashSet<OfflinePlayer>()));
		}
	 ...
	 * 
	 * @param monger
	 *            The person who initiated the war
	 * @param target
	 *            The person upon which the war was declared
	 ...
	 * My hope is this will not cause a problem because of reading the list from
	 * disk and then writing to the same config through addWar
	 */
		...
		if (configFile.exists()) {
			...
			while (kitr.hasNext()) {
				...
				if (config.isList(mongerName)) {
					Iterator<String> titr = config.getStringList(mongerName)
							.iterator();
					while (titr.hasNext()) {
						String targetName = titr.next();
						OfflinePlayer target = getServer().getOfflinePlayer(
								targetName);
	...
	/**
	 * Saves the current warMongers and their targets to a config file in the
	 * directory
	 */
	private void saveMongersFile() {
		...
		try {
			...
			log.log(Level.WARNING, "Unable to save the configuration to disk: "
	...
	/**
	 * Checks to see if a player has declared war on another player This differs
	 * from areAtWar in that it will not return true if the target had declared
	 * war on the monger
	 * 
	 * @param monger
	 *            The player who may have declared war
	 * @param target
	 *            The player whom may have had war declared upon them
